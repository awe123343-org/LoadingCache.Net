...
def snapshot(root, project_directory):
    ...
    paths.update(
        path
        for path in (STAGED / "tools/LoadingCache.ServiceProbe").iterdir()
...
def verify_core(directory, root=None):
    ...
    require(
        record["schemaVersion"] == 1 and record["kind"] == "core-build", "Wrong core build manifest"
    )
    require(
        record["status"] == "passed" and record["exitCode"] == 0 and record["sourceUnchanged"],
        "Core build did not pass",
    )
    require(record["sourceBefore"] == record["sourceAfter"], "Core sources changed during build")
    require(
        set(record["sourceCopies"]) == set(record["sourceBefore"]), "Core source copies incomplete"
    )
    require(
        all(
            sha(directory / record["sourceCopies"][name]) == digest
            for name, digest in record["sourceBefore"].items()
        ),
        "Frozen core source differs from build",
    )
    require(
        all(sha(directory / name) == digest for name, digest in record["outputs"].items()),
        "Frozen core output differs from build",
    )
    require(
        sha(directory / "build.log") == record["logSha256"]
        and sha(directory / "sdk.log") == record["sdkInfoSha256"],
        "Core build log/SDK evidence changed",
    )
    require(
        record["sourceBefore"]["src/LoadingCache/EngineTimeout.cs"] == TIMEOUT_SOURCE_SHA256,
        "Cache timeout origin sequence requires source re-review",
    )
    if root is not None:
        ...
        inputs = {
            ...
            and (
                name.startswith("src/LoadingCache/")
                and name.endswith((".cs", ".csproj", ".props", ".targets"))
                or name in config
            )
        }
        inputs.update(
            (
                "src/LoadingCache/obj/project.assets.json",
                "src/LoadingCache/obj/LoadingCache.csproj.nuget.g.props",
                "src/LoadingCache/obj/LoadingCache.csproj.nuget.g.targets",
            )
        )
        inputs.update(
            str(path.relative_to(root))
            for path in ancestor_configs(root, root / "src/LoadingCache")
        )
        require(
            inputs == set(record["sourceBefore"]),
            "Current core source/config inventory differs from build",
        )
        require(
            all(sha(root / name) == digest for name, digest in record["sourceBefore"].items()),
            "Current core source/config differs from build",
        )
    return record
...
def verify(directory, root=None, built=None):
    record = json.loads((directory / "manifest.json").read_text())
    require(
        record["schemaVersion"] == 1 and record["kind"] == "loading-probe-build",
        "Wrong probe build manifest",
    )
    require(record["status"] == "passed" and record["exitCode"] == 0, "Probe build did not pass")
    require(
        record["sourceBefore"] == record["sourceAfter"], "Probe sources/config changed during build"
    )
    require(
        set(record["sourceCopies"]) == set(record["sourceBefore"]), "Probe source copies incomplete"
    )
    require(
        all(
            sha(directory / record["sourceCopies"][name]) == digest
        ...
        "Frozen probe source differs from build",
    )
    require(
        all(sha(directory / name) == digest for name, digest in record["buildInputs"].items()),
        "Probe build inputs changed",
    )
    require(
        all(sha(directory / name) == digest for name, digest in record["outputs"].items()),
        "Probe output differs from build",
    )
    require(
        sha(directory / "build.log") == record["logSha256"]
        and sha(directory / "sdk.log") == record["sdkInfoSha256"],
        "Probe build log/SDK evidence changed",
    )
    core = verify_core(directory / "core", root)
    require(
        sha(directory / "core/manifest.json") == record["coreManifestSha256"],
        "Core manifest link changed",
    )
    require(
        record["outputs"]["bin/LoadingCache.dll"] == core["outputs"]["bin/LoadingCache.dll"],
        "Probe used a different core binary",
    )
    for name, digest in core["sourceBefore"].items():
        source = str(Path(core["sourceRoot"]) / name)
        if source in record["sourceBefore"]:
            require(
                record["sourceBefore"][source] == digest, "Probe/core source attribution differs"
            )
    if root is not None:
        require(root == Path(record["sourceRoot"]), "Probe build source root differs")
        require(
            snapshot(root, Path(record["projectDirectory"])) == record["sourceBefore"],
            "Current source/config differs from probe build",
        )
    if built is not None:
        require(
            {"bin/" + path.name: sha(path) for path in built.iterdir() if path.is_file()}
            == record["outputs"],
            "Supplied binaries differ from probe build",
        )
    return record
...
''',
    ...
    record = {
        "schemaVersion": 1,
        "kind": "loading-probe-build",
        "sourceRoot": str(root),
        "projectDirectory": str(output),
        "framework": args.framework,
        "command": command,
        "sourceBefore": before,
        "sourceCopies": copies,
        "coreManifestSha256": sha(output / "core/manifest.json"),
        "buildInputs": {project.name: sha(project)},
        "status": "running",
    }
    write(output / "manifest.json", record)
    sdk = subprocess.run(
        [str(args.dotnet.resolve()), "--info"], cwd=root, capture_output=True, text=True, check=True
    )
    ...
    record.update(
        ...
        sourceAfter=snapshot(root, output),
        logSha256=sha(output / "build.log"),
        sdkInfoSha256=sha(output / "sdk.log"),
    )
