    ...
    names = (
        subprocess.check_output(
            [
                ...
                "--",
                "src",
                "tools/LoadingCache.ServiceProbe",
                "Directory.Build.props",
                "Directory.Build.targets",
                "Directory.Packages.props",
                "global.json",
                "NuGet.Config",
                "docs/v1-service-profile.md",
            ],
            cwd=ROOT,
        )
        .decode()
...
def metrics(result, profile, backend):
    ...
    for name, seconds in (("warmup", profile["warmupSeconds"]), ("measured", profile["seconds"])):
        ...
        require(
            all(value in ("completed", "rejected", "timeout", "failed") for value in outcomes),
            "Unknown request outcome",
        )
        require(
            all(isinstance(value, int) and value >= 0 for value in ticks), "Invalid request latency"
        )
        ...
        successful = sorted(
            value for value, outcome in zip(ticks, outcomes, strict=True) if outcome == "completed"
        )
        expected_p99 = (
            successful[max(0, math.ceil(len(successful) * 0.99) - 1)] * (1_000_000 / frequency)
            if successful
            else None
        )
        require(phase["P99Microseconds"] == expected_p99, f"Wrong {name} p99")
        require(
            math.isfinite(phase["WallSecondsIncludingDrain"])
            and phase["WallSecondsIncludingDrain"] > 0,
            "Invalid phase duration",
        )
    measured = result["measured"]
    server = result["server"]
    require(
        math.isfinite(server["cpuSeconds"]) and server["cpuSeconds"] > 0, "Invalid server CPU time"
    )
    require(
        server["capacity"] == 1024
        and server["payloadCharacters"] == 1024
        and server["lookupsPerRequest"] == 1,
        "Server workload changed",
    )
    require(
        server["loaderServiceTimeMilliseconds"] == server["expectedLoads"] == 0,
        "Server loader profile changed",
    )
    if backend == "cache":
        drain = server["drain"]
        ...
        require(
            ...
            "Final cache state was not quiescent",
        )
    else:
    ...
    return {
        "valid": valid,
        "cpuSecondsPerRequest": server["cpuSeconds"] / measured["Completed"]
...
def main():
    ...
    try:
        ...
        require(
            (built / "LoadingCache.ServiceProbe.dll").is_file(),
            "Build the requested framework in Release before running",
        )
        ...
        runtime_result = subprocess.run(
            [str(dotnet), "--list-runtimes"], capture_output=True, text=True, timeout=30
        )
        (output / "runtime-list.log").write_text(
            runtime_result.stdout + runtime_result.stderr, encoding="utf-8"
        )
        ...
        for framework in ("Microsoft.NETCore.App", "Microsoft.AspNetCore.App"):
            matches = re.findall(
                r"^" + re.escape(framework) + r" (\S+) \[(.+)\]$",
                runtime_result.stdout,
                re.MULTILINE,
            )
            matches = [
                (version, Path(base) / version)
                for version, base in matches
                if version.startswith(family)
            ]
            require(
                len(matches) == 1,
                f"Select an installation with exactly one {framework} patch for {family}",
            )
        ...
        for directory in (core_path, aspnet_path, dotnet.parent / "host/fxr"):
            runtime_files.update(
                {str(path): sha(path) for path in sorted(directory.rglob("*")) if path.is_file()}
            )
        runtime_environment = {
            key: value
        ...
        require(
            all(key in harmless for key in runtime_environment),
            f"Runtime overrides are incompatible with the default-runtime profile: {sorted(set(runtime_environment) - harmless)}",
        )
        manifest = {
            "schemaVersion": 1,
            "argv": sys.argv,
            ...
            "profile": {
                "rate": 2000,
                "seconds": 2 if args.smoke else 30,
                "warmupSeconds": 1 if args.smoke else 10,
                "maxPending": 256,
        ...
        def verify_inputs():
            require(
                {name: sha(ROOT / name) for name in source_paths()} == manifest["sources"],
                "Candidate source paths or bytes changed",
            )
            require(
                file_manifest(inputs) == manifest["inputs"], "Frozen source/binary bytes changed"
            )
            require(
                all(sha(Path(path)) == digest for path, digest in runtime_files.items()),
                "Runtime installation bytes changed",
            )
        ...
        def run(label, backend, expiry="ttl", statistics=False):
            ...
            with (directory / "server.log").open("w") as log:
                server = subprocess.Popen(
                    server_command, stdout=log, stderr=subprocess.STDOUT, cwd=ROOT, env=environment
                )
                ...
                try:
                    ...
                    command = [
                        ...
                        str(profile["seconds"]),
                        "--warmup-seconds",
                        str(profile["warmupSeconds"]),
                        "--max-pending",
                        str(profile["maxPending"]),
                        "--timeout-ms",
                        str(profile["timeoutMilliseconds"]),
                    ]
                    ...
                    with (directory / "client.log").open("w") as client_log:
                        result_process = subprocess.run(
                            ...
                            cwd=ROOT,
                            timeout=profile["seconds"] + profile["warmupSeconds"] + 90,
                        )
                    commands.update(
                        clientExitCode=result_process.returncode,
                        clientElapsedSeconds=time.monotonic() - started,
                    )
                    write(directory / "commands.json", commands)
                    ...
                    require(
                        server_result["runtime"] == manifest["expectedRuntime"],
                        "Actual runtime did not match selected framework/patch",
                    )
                    for field, expected in (
                        ("cacheAssemblySha256", "expectedCacheSha256"),
                        ("probeAssemblySha256", "expectedProbeSha256"),
                        ("coreLibrarySha256", "expectedCoreLibrarySha256"),
                        ("aspNetCoreAssemblySha256", "expectedAspNetCoreAssemblySha256"),
                    ):
                        require(
                            server_result[field].lower() == manifest[expected],
                            f"Executing {field} differs from frozen inputs",
                        )
                    require(
                        (
                            server_result["backend"],
                            server_result["expiry"],
                            server_result["statistics"],
                        )
                        == (backend, expiry, statistics),
                        "Server executed a different profile arm",
                    )
                    record = {
                        "label": label,
                        "backend": backend,
                        ...
                        "rawSha256": sha(raw),
                        **metrics(result, profile, backend),
                    }
        ...
        def compare(first, second, precision):
            ...
            return {
                ...
                **{
                    metric: (
                        abs(second[metric] / first[metric] - 1)
                        if precision
                        else second[metric] / first[metric] - 1
                    )
                    if valid
                    else None
        ...
        for index in range(rounds):
            aa.append(
                compare(run(f"aa-{index}-a", "control"), run(f"aa-{index}-b", "control"), True)
            )
        pairs = []
            ...
            for statistics in (False, True):
                for index in range(rounds):
                    ...
                    for backend in ("control", "cache") if index % 2 == 0 else ("cache", "control"):
                        paired[backend] = run(
                            f"{expiry}-stats{int(statistics)}-{index}-{backend}",
                            backend,
                            expiry,
                            statistics,
                        )
        ...
        summary.update(
            ...
            aaPrecisionWithinFivePercent=precise,
            allPairsWithinFivePercent=passed,
            aa=aa,
            pairs=pairs,
        )
        ...
        write(output / "summary.json", summary)
        write(
            output / "raw-hashes.json",
            {
                str(path.relative_to(output)): sha(path)
                for path in sorted(output.rglob("*.json"))
                if path.name != "raw-hashes.json"
            },
        )
