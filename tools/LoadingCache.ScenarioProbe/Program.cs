...
internal static class Program
{
    ...
    private static async Task Main(string[] args)
    {
        ...
        var selected = options.Scenario == "all" ? Names : [options.Scenario];
        if (
            selected.Any(name =>
        ...
        var report = new
        {
            ...
            cacheSha256 = Hash(typeof(CacheApi.CacheBuilder).Assembly.Location),
    ...
    {
        if (name is "sync-fanin-contract" or "manual-sync-fanin-contract")
            return SyncFanIn(name, options, index);
        if (name is "weak-key-cleanup" or "weak-value-cleanup")
            return ReferenceCleanup(name, options, index);
        var clock = new ManualClock();
        var trace = options.TraceFile is null
            ? CreateTrace(options.Cycles, options.Seed, options.Capacity * 4)
            : JsonSerializer.Deserialize<int[]>(File.ReadAllText(options.TraceFile))
        ...
        int expectedWeightChanges =
            name == "weight-replace"
                ...
                : 0;
        long callbacks = 0;
        var builder = CacheApi
            .CacheBuilder.Create<Key, Value>()
        ...
        if (options.Statistics && name.StartsWith("weight", StringComparison.Ordinal))
            builder.RecordStatistics();
        if (name is "ttl" or "ttl-cleanup" or "runtime-expiry")
            builder.ExpireAfterWrite(Duration);
        if (name is "tti" or "tti-cleanup" or "runtime-access")
        ...
        if (name is "refresh-auto" or "runtime-refresh")
            builder.RefreshAfterWrite(Duration);
        if (name == "weak-key-lookup")
            builder.WeakKeys();
        if (name == "weak-value-lookup")
            builder.WeakValues();
        if (name == "eviction-listener")
            builder.EvictionListener(_ => Interlocked.Increment(ref callbacks));
        if (name == "removal-listener")
            builder
                .NotificationCapacity(options.Cycles + 16)
        ...
        CacheApi.ICache<Key, Value>? sync = null;
        CacheApi.IAsyncCache<Key, Value>? asyncCache = null;
        if (asyncMode)
        ...
        try
        {
            ...
            for (int cycle = 0; cycle < options.Cycles; cycle++)
            {
                ...
                switch (name)
                {
                    ...
                    case "manual-sync-miss":
                        ...
                        var loaded =
                            name == "sync-miss"
                                ? ((CacheApi.ILoadingCache<Key, Value>)sync).Get(keys[0])
                    ...
                    case "manual-async-gated-fanin":
                        ...
                        {
                            requests[waiter] = asyncCache
                                is CacheApi.IAsyncLoadingCache<Key, Value> loading
                    ...
                    case "runtime-refresh":
                        ...
                        sync!.Put(keys[0], alternate[0]);
                        operations++;
                        var refreshing = (CacheApi.ILoadingCache<Key, Value>)sync;
                    ...
                    case "prefetch-async":
                        ...
                        if (asyncMode)
                        {
                            asyncCache!.Clear();
                            operations++;
                            var result = await ((CacheApi.IAsyncLoadingCache<Key, Value>)asyncCache)
                                .GetAllAsync(batch)
                                .ConfigureAwait(false);
                        ...
                        else
                        {
                            sync!.Clear();
                            operations++;
                            var result = ((CacheApi.ILoadingCache<Key, Value>)sync).GetAll(batch);
                            ValidateBulk(result, values, ref checksum);
                            operations++;
                        }
            ...
            if (name == "removal-listener")
                await UntilAsync(() => Interlocked.Read(ref callbacks) == options.Cycles - 1)
                    .ConfigureAwait(false);
            ...
            if (!options.Statistics)
                Check(stats.Hits == 0 && stats.Misses == 0, "statistics disabled");
            ...
            if (options.Statistics && name == "quiet-lookup")
                Check(stats.Hits == 1 && stats.Misses == 0, "quiet lookup altered statistics");
            Check(
    ...
    private static void Read(
        CacheApi.ICache<Key, Value> cache,
        Key key,
        Value expected,
        bool present,
        ref long operations,
    ...
    private static Sample ReferenceCleanup(string name, Options options, int index)
    {
        ...
        var (weak, retained) = PopulateUnrooted(cache, name == "weak-key-cleanup");
    ...
    {
        using var invoked = new CountdownEvent(options.FanIn);
        using var completed = new CountdownEvent(options.FanIn);
        using var entered = new ManualResetEventSlim();
        using var release = new ManualResetEventSlim();
        long calls = 0;
        var key = new Key(0);
        var expected = new Value(1, 1);
        Value Load(Key _)
        {
            Interlocked.Increment(ref calls);
            entered.Set();
            Check(release.Wait(TimeSpan.FromSeconds(30)), "loader gate timeout");
            return expected;
        }
        var builder = CacheApi
            .CacheBuilder.Create<Key, Value>()
        ...
        if (options.Statistics)
            builder.RecordStatistics();
        using var cache =
            name == "sync-fanin-contract" ? builder.BuildLoading(Load) : builder.Build();
        var values = new Value?[options.FanIn];
        var failures = new System.Collections.Concurrent.ConcurrentQueue<Exception>();
        var threads = new Thread[options.FanIn];
        var utcStart = DateTimeOffset.UtcNow;
        ...
        try
        {
            for (int i = 0; i < threads.Length; i++)
            {
                ...
                threads[i] = new Thread(() =>
                {
                    ...
                    try
                    {
                        values[worker] = cache is CacheApi.ILoadingCache<Key, Value> loading
                            ? loading.Get(key)
                            : cache.GetOrAdd(key, Load);
                    }
            ...
            Check(
                invoked.Wait(TimeSpan.FromSeconds(30)) && entered.Wait(TimeSpan.FromSeconds(30)),
                "callers did not enter"
            );
            Check(completed.CurrentCount == options.FanIn, "a gated call completed before release");
            release.Set();
            Check(completed.Wait(TimeSpan.FromSeconds(30)), "callers failed to finish");
            Check(
                failures.IsEmpty
                    && calls == 1
                    && values.All(value => ReferenceEquals(value, expected)),
                "sync fan-in result/backend"
            );
            int drains = Drain(cache);
        ...
        finally
        {
            ...
            foreach (var thread in threads)
                if (thread is not null)
                    Check(thread.Join(TimeSpan.FromSeconds(30)), "sync caller did not terminate");
        }
    }
    ...
    private sealed class Expiry : CacheApi.IExpiry<Key, Value>
    {
    ...
    private sealed class Loader(Value[] values, bool prefetch, bool gated)
        : CacheApi.IBulkSyncCacheLoader<Key, Value>,
            CacheApi.IBulkAsyncCacheLoader<Key, Value>
    {
        private long _calls,
            _bulkCalls;
    ...
    private sealed record Sample(
        ...
        int DrainPasses,
        DateTimeOffset UtcStarted,
        DateTimeOffset UtcEnded,
    ...
    )
    {
        public static Options Parse(string[] args)
        {
            ...
            Check(
                (map.GetValueOrDefault("--scenario") == "trace-policy")
