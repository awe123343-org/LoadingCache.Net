...
def main():
    ...
    for project in projects:
            ...
            missing.append(
                {
                    ...
                    "reason": "No existing project.assets.json; not restored by this inventory",
                }
            )
            continue
        ...
        graphs.append(
            {
                ...
                "assets": str(assets.relative_to(ROOT)),
                "recordedProjectPath": restore.get("projectPath"),
                "restoreSources": list(restore.get("sources", {})),
        ...
        if Path(restore.get("projectPath", "")).resolve() != project.resolve():
            missing.append(
                {
                    "project": project_name,
                    "reason": "Restore graph records a different project path",
                }
            )
        ...
        for target, entries in data.get("targets", {}).items():
            ...
            direct = {
                name.casefold(): info for name, info in framework.get("dependencies", {}).items()
            }
            for library, entry in entries.items():
                ...
                runtime_assets = any(
                    not filename.endswith("_._")
                    for kind in ("runtime", "native", "runtimeTargets")
                    for filename in kinds.get(kind, [])
                )
                shipped = (
                    area == "src"
                    and runtime_assets
                    and (dependency or {}).get("suppressParent") != "All"
                )
                scope = (
                    ("core-shipped" if project.parent.name == "LoadingCache" else "di-shipped")
                    if shipped
                    else "production-build-only"
                    if area == "src"
                    else {
                        "tests": "test",
                        "samples": "sample",
                ...
                if (
                    ...
                    and (
                        (dependency or {}).get("suppressParent") == "All"
                        or (dependency or {}).get("autoReferenced")
                    )
                ):
                    scope += "-build-only"
                add(
                    ...
                    roots,
                    {
                        ...
                        "target": target,
                        "scope": scope,
                        "relationship": "sdk-auto-reference"
                        if (dependency or {}).get("autoReferenced")
                        else "direct-restore-root"
                        if dependency
                        else "transitive",
                        "privateAssets": (dependency or {}).get("suppressParent"),
                        "assetKinds": kinds,
                        "dependencies": entry.get("dependencies", {}),
                    },
                    data["libraries"].get(library, {}).get("sha512"),
                )
                ...
                if match and match[1] == match[2]:
                    add(
                        ...
                        roots,
                        {
                            "project": project_name,
                            "target": target,
                            "scope": "sdk-download-build-only",
                            "relationship": "downloadDependency",
                        },
                    )
                else:
                    missing.append(
                        {
                            "project": project_name,
                            "download": download,
                            "reason": "Download dependency does not record a single resolved version",
                        }
                    )
    ...
    for (lower_id, version), package in sorted(packages.items()):
        ...
        with archive.open("rb") as stream:
            package["archiveSha512"] = base64.b64encode(
        ...
        package["archiveMatchesSidecar"] = (
            package["archiveSha512"] == package["archiveSha512Sidecar"]
            if sidecar.is_file()
            else None
        )
        ...
        package["lockHashesMatchRecordedContentHash"] = (
            all(value == content_hash for value in package["expectedSha512"])
            if package["expectedSha512"] and content_hash
            else None
        )
        if package["archiveMatchesSidecar"] is False:
            missing.append(
                {
                    "package": package["id"] + "/" + version,
                    "reason": "Raw archive SHA512 differs from its cached .nupkg.sha512 sidecar; distinct from the separately recorded NuGet contentHash. Origin of this metadata discrepancy is not established; no security or tampering conclusion.",
                }
            )
        if package["lockHashesMatchRecordedContentHash"] is False:
            missing.append(
                {
                    "package": package["id"] + "/" + version,
                    "reason": "Restore-lock hash differs from recorded NuGet contentHash; graph/cache identity needs review",
                }
            )
        ...
        with zipfile.ZipFile(archive) as zipped:
            ...
            package["metadata"] = {
                ...
                "licenseValue": license_node.text if license_node is not None else None,
                "legacyLicenseUrl": metadata.findtext("{*}licenseUrl"),
            ...
            legal = [
                ...
                and re.search(
                    r"^(license|licence|copying|notice|third[-_. ]?party)",
                    Path(name).name,
                    re.IGNORECASE,
                )
            ]
            if package["metadata"]["licenseType"] == "file":
                ...
                if named not in zipped.namelist():
                    missing.append(
                        {
                            "package": package["id"] + "/" + version,
                            "reason": "Declared license file absent",
                            "file": named,
                        }
                    )
            ...
            for index, name in enumerate(sorted(legal)):
                ...
                package["licenseAndNoticeFiles"].append(
                    {
                        "archivePath": name,
                        "sha256": digest(content),
                        "preservedPath": str(preserved.relative_to(output)),
                    }
                )
            if lower_id.startswith("loadingcache"):
                package["localPackageRootNoticeComparison"] = {
                    name: {
                        "archiveSha256": digest(zipped.read(name))
                        if name in zipped.namelist()
                        else None,
                        "currentRootSha256": captured[name],
                        "sameBytes": name in zipped.namelist()
                        and digest(zipped.read(name)) == captured[name],
                    }
            ...
            if license_node is None:
                missing.append(
                    {
                        "package": package["id"] + "/" + version,
                        "reason": "No modern nuspec license field; legacy URL and any preserved legal files require manual review",
                    }
                )
        resolved.append(package)
    ...
    for name in ADAPTATIONS:
        ...
        adaptation_rows.append(
            {
                "file": name,
                "sha256": captured[name],
                "stableRevisionPresent": STABLE in text,
                "copyrightPresent": "Copyright" in text and "Ben Manes" in text,
                "noticeMentionsFile": Path(name).name in notice,
                "localModificationMarker": any(marker in text for marker in (".NET", "CLR")),
                "masterRevision": "d885a95eee51fdfe13f450fd9cba80f58f7e0def"
                if "d885a95eee51fdfe13f450fd9cba80f58f7e0def" in text
                else None,
                "dougLeaAttribution": "Doug Lea" in text,
            }
        )
    ...
    result = {
        ...
        "argv": sys.argv,
        "scope": "Existing NuGet project restore graphs plus explicit download dependencies and repository-local dotnet tool pin; no restore or legal clearance",
        "inputsUnchanged": stable,
        "inputs": captured,
        "graphs": graphs,
        "packages": resolved,
        "missingOrNeedsReview": missing,
    ...
    lines = [
        ...
        "",
        f"Captured {result['generatedUtc']}: {len(graphs)} existing project restore graphs, {len(resolved)} distinct NuGet ID/version pairs. Inputs unchanged while captured: **{stable}**.",
        "",
        ...
        "",
        f"Raw graph/nuspec/license copies, all project/TFM usages, dependency edges, archive SHA256/SHA512, source-feed metadata and input hashes: `{output.relative_to(ROOT)}/inventory.json`. Evidence hashes are in the adjacent `evidence-manifest.json`.",
        "",
            ...
            sorted(
                {
                    ...
                    if usage["scope"].endswith("-shipped")
                }
            ),
        )
    ...
    for package, scopes in shipped:
        lines.append(
            f"- {package['id']} {package['version']}: {', '.join(scopes)}; license `{package.get('metadata', {}).get('licenseValue', 'missing')}`."
        )
    lines += [
        "",
    ...
    for package in resolved:
        ...
        license_text = (
            f"{metadata.get('licenseType')}: {metadata.get('licenseValue')}"
            if metadata.get("licenseType")
            else "legacy URL / review required"
            if metadata.get("legacyLicenseUrl")
            else "missing"
        )
        scopes = sorted({usage["scope"] for usage in package["usages"]})
        lines.append(
            f"| {package['id']} | {package['version']} | {', '.join(scopes)} | {license_text.replace('|', '&#124;')} | `{package.get('archiveSha256', 'missing')}` |"
        )
    lines += ["", "## Missing metadata and review boundaries", ""]
    lines += ["- " + json.dumps(item, ensure_ascii=False) for item in missing] or [
        "- No missing cache archives or modern license declarations were found in the enumerated graphs."
    ]
    lines += [
        "",
        ...
        "",
        "The local headers and notices were inspected, not re-downloaded or compared byte-for-byte with every upstream file. All four cite Caffeine stable `"
        + STABLE
    ...
    for row in adaptation_rows:
        checks = [
            row[key]
            for key in (
                "stableRevisionPresent",
                "copyrightPresent",
                "noticeMentionsFile",
                "localModificationMarker",
            )
        ]
    ...
    print(
        json.dumps(
            {
                "projects": len(graphs),
                "packages": len(resolved),
                "needsReview": len(missing),
                "inputsUnchanged": stable,
                "shipped": [package["id"] + "/" + package["version"] for package, _ in shipped],
            }
        )
    )
    if not stable:
        raise SystemExit(
            "Inputs changed during capture; keep this evidence but rerun on a stable snapshot."
        )
