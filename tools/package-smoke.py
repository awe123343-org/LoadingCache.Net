    ...
    try:
        run(
            ...
            [
                "dotnet",
                "pack",
                "src/LoadingCache/LoadingCache.csproj",
                "-c",
                "Release",
                "-o",
                str(feed),
                "-p:IsPackable=true",
                "-p:PackageId=LoadingCache.LocalValidation",
                "-p:PackageVersion=0.1.0-alpha.localvalidation",
        ...
        with zipfile.ZipFile(archive) as package:
            ...
            if dependencies:
                raise RuntimeError(
                    f"Core must have no runtime package dependencies: {[item.attrib for item in dependencies]}"
                )
        ...
        report["artifact_sha256"] = {
            item.name: hashlib.sha256(item.read_bytes()).hexdigest() for item in (archive, symbols)
        }
        ...
        assets = json.loads(
            (root / "tests/LoadingCache.PackageTests/obj/project.assets.json").read_text()
        )
        ...
        run(
            ...
            [
                ...
                str(
                    root
                    / "tests/LoadingCache.PackageTests/bin/Release/net8.0/LoadingCache.PackageTests.dll"
                ),
            ],
        )
        run(
