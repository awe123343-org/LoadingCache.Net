...
public sealed class ReadBufferDiagnosticsTests
{
    ...
    {
        using StripedReadBuffer<int> buffer = new(2, 2);
        buffer.TryOffer(1).Should().Be(ReadBufferOfferResult.Success);
        int stripes = buffer.DiagnosticStripeCountForTesting;
        for (int stripe = 0; stripe < stripes; stripe++)
        {
            buffer.AddDropStatisticsForTesting(stripe, droppedFull: stripe + 1, droppedFailed: 1);
        }
    ...
    {
        using StripedReadBuffer<int> buffer = new(1, 4, recordStatistics: false);
        buffer.TryOffer(1).Should().Be(ReadBufferOfferResult.Success);
        buffer.SetForcedCasFailuresForTesting(3);
        buffer.TryOffer(2).Should().Be(ReadBufferOfferResult.Failed);
    ...
    )
    {
        ...
        using Cache<int, int> cache = new(engine);
        cache.Put(1, 1);
        scheduler.RunAll();
